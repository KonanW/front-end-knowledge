...
function isObject( target ){
  return typeof target ===' object' && obj !==null;
}
...
function deepcopy(source) {
 ...
 for(keys in source) {
   if(Object.prototype.hasOwnProperty(keys,source)){
     if(isObject(source[key])) {
       target[key] = deepcpcopy(source[key]);
     }else{
...
function deepcopy2(source, list) {
 ...
 for(keys in source) {
   if(Object.prototype.hasOwnProperty(keys,source)){
     if(isObject(source[key])) {
       target[key] = deepcpcopy2(source[key], list);
     }else{
       target[key] = source[key];
     }
   }
