 ...
 */
  ...
  * 这里新旧Virtual Dom只会在同级的元素进行对比，利用递归实现深度优先遍历 －－ patch
  */

  // patch.js

   return function patch(old,vnode,hydrating,removeOnly,parentElm,refElm){
     //
     if(!isRealElement && sameVnode(old,vnode)) {
       //
       patchVnode(old,vnode,insertedVnodeQueue,removeOnly)
     }else {
       if(isRealElement) {
         old = emptyNodeAt(old);
       }
